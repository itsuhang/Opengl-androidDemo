...
public class OpenGlRenderTen implements GLSurfaceView.Renderer, CanTranform {
    ...
    private int mLightCutoff;
    private int mLightOutcutoff;
    ...
    public OpenGlRenderTen(Context context) {
        mContext = context;
    }
    ...
    private void getLocations() {
        ...
        mLightCutoff = GLES30.glGetUniformLocation(mObjectProgram, "light.cutoff");
        mLightOutcutoff = GLES30.glGetUniformLocation(mObjectProgram, "light.outcutoff");
    ...
    private void init() {
        mObjectProgram = ShaderUtil.createProgram(ShaderUtil.createShader(mContext, R.raw.vertex_shader_ten, GLES30.GL_VERTEX_SHADER), ShaderUtil.createShader(mContext, R.raw.fragment_shader_ten, GLES30.GL_FRAGMENT_SHADER));
    ...
    private void bindMatrix() {
        ...
        GLES30.glUniform3f(mLightDirection, mCamera.front[0], mCamera.front[1], mCamera.front[2]);
        GLES30.glUniform3f(mLightPosition, mCamera.position[0], mCamera.position[1], mCamera.position[2]);
        GLES30.glUniform1f(mLightCutoff, (float) Math.cos(VectorUtil.angleTransform(10.5f)));
        GLES30.glUniform1f(mLightOutcutoff, (float) Math.cos(VectorUtil.angleTransform(15.5f)));
